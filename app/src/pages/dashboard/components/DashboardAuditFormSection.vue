...
const emit = defineEmits<{
  (e: 'update:target-url', value: string): void
  (e: 'update:selected-profile', value: 'wad' | 'eaa'): void
  (e: 'start-audit'): void
}>()

const onTargetUrlInput = (event: Event) => {
  const element = event.target as HTMLInputElement
  emit('update:target-url', element.value)
}

const onSelectedProfileInput = (event: Event) => {
  const element = event.target as HTMLInputElement
  const value = element.value === 'eaa' ? 'eaa' : 'wad'
  emit('update:selected-profile', value)
}
</script>
...
<template>
  <section class="panel audit-form">
    ...
    <div class="form-grid">
      <div class="field">
        ...
        <div class="input-row">
          <input
            ...
            @keyup.enter="emit('start-audit')"
          />
          <button class="btn btn-primary" @click="emit('start-audit')" :disabled="!canRunAudit">
            <span v-if="auditLoading" class="spinner-border spinner-border-sm"></span>
            {{ auditLoading ? 'Auditujem...' : 'Analyzovať web' }}
          </button>
        </div>
      ...
      <div v-if="auditError" class="form-error">{{ auditError }}</div>
